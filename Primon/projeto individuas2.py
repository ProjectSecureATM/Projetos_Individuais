        ...
        if self.current_os == 'Windows':
            try:
                ...
                for index, device in enumerate(devices):
                    ...
                    result = cursor_sql_server.fetchone()
                    if result:
                        fk_componente = result[0]
                    else:
                        cursor_sql_server.execute(
                            "INSERT INTO CodigoComponentes (Componente) OUTPUT INSERTED.idCodComponentes VALUES (%s)",
                            (porta,)
                        )
                        fk_componente = cursor_sql_server.fetchone()[0]

                    cursor_sql_server.execute(
                        "INSERT INTO DescricaoComponentes (produto, fabricante, dataDia, fkComponente) VALUES (%s, %s, GETDATE(), %s)",
                        (produto, fabricante, fk_componente)
                    )
